...
const PreviewCard = ({ data }) => {
  ...
  return (
    <div
      className={`min-h-48 shadow-3xl rounded text-center m-5 p-8 hover:scale-105 transition ease-in-out duration-300 text-darkGrey flex flex-col justify-between cursor-pointer ${ticketPriorityBorderColor(
        priority
      )}`}
      onClick={clicked}>
      <p className="text-xl font-semibold">{name}</p>
      <p>{details}</p>
      <p className="text-grey">{`Priority: ${ticketPriorityLevel(
        priority
      )}`}</p>
